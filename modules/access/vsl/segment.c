...
static int Open( vlc_object_t *p_this )
{
    ...
    for( int i = 0; i < 3; ++i )
    {
        ...
        p_sys->i_size = stream_Size( p_sys->p_stream );
        if( p_sys->i_size <= 0 )
        {
            ...
            p_sys->i_size = vsl_cbGetBytes( &p_sys->cb, p_sys->i_order );
            if( p_sys->i_size <= 0 )
            {
            ...
            if( p_sys->i_size < 64 * 1000 && p_sys->i_duration_milli > 10 * 1000 )
            {
...
STREAM_OPEN_ERROR:
    ...
    {
        p_sys->i_bytes_per_second = p_sys->i_size * 1000 / p_sys->i_duration_milli;
    }
...
static ssize_t Read( access_t *p_access, uint8_t *p_buffer, size_t i_len )
{
    ...
    if( p_sys->i_size != 0 )
    {
        ...
        uint64_t i_remaining = p_sys->i_size - p_access->info.i_pos;
    ...
    assert( i_read_ret > 0 );
    assert( p_access->info.i_pos <= p_sys->i_size );
    assert( p_access->info.i_pos + i_read_ret <= p_sys->i_size );
...
static int SkipSeekedUnknownHeader( access_t *p_access, stream_t *p_stream, uint64_t i_seek_pos )
{
    ...
    int64_t i_whole_remain = p_sys->i_size - i_seek_pos;
...
static int SkipSeekedFlvHeader( access_t *p_access, stream_t *p_stream, uint64_t i_seek_pos )
{
    ...
    uint64_t i_whole_remain = p_sys->i_size - i_seek_pos;
    if( i_stream_len < i_whole_remain )
    {
        msg_Err( p_access, "flvhead: too small seeked size: %"PRId64" < %"PRId64,
                i_stream_len,
...
static stream_t *stream_vslNewYoukuSeek( access_t* p_access, uint64_t i_pos )
{
    ...
    if( p_sys->i_size <= 0 )
    {
        msg_Err( p_access, "segment: youku: seek: invalid i_size < 0" );
        goto EXIT_FAIL;
    }
    ...
    do {
        ...
        int64_t i_seeked = p_sys->i_size - i_stream_len;
        if( i_seeked < 0 )
        {
...
static int Seek( access_t *p_access, uint64_t i_pos )
{
    ...
    msg_Info( p_access, "segment: seek %"PRId64"/%"PRId64, i_pos, p_sys->i_size );

    if( i_pos > p_sys->i_size )
        goto EXIT_FAIL;
    ...
    {
        uint64_t i_seek_forward = i_pos - p_access->info.i_pos;
        int i_seek_ret = stream_ReadSeek( p_sys->p_stream, i_seek_forward );
        if( i_seek_ret <= 0 )
            goto EXIT_FAIL;

        p_access->info.i_pos += i_seek_ret;
        p_access->info.b_eof = ( p_access->info.i_pos >= p_sys->i_size );
        return VLC_SUCCESS;
    }
    ...
    p_access->info.i_pos = i_pos;
    p_access->info.b_eof = ( i_pos >= p_sys->i_size );
