...
// 32K/s
#define BYTES_PER_READ ( 8 * 1024 )
#define MAX_DELAY_PER_READ ( 1000 * 1000 / 4 )
...
#define msg_RBuf( p_this, ... )
#define msg_WBuf( p_this, ... )
...
static bool StreamNeedAbort( stream_t *p_stream )
{
    stream_sys_t *p_sys = p_stream->p_sys;
    if( p_sys->b_abort || p_sys->b_error || !vlc_object_alive( p_stream ) )
        return true;
...
static int WaitBufferForRead_l( stream_t *p_stream, int i_read )
{
    ...
    {
        if( StreamNeedAbort( p_stream ) ) {
            msg_Warn( p_stream, "ringbuf: WaitBufferForRead_l need abort" );
            i_data_ready = -1;
            break;
        }
        ...
        // msg_Warn( p_stream, "ringbuf: WaitBufferForRead_l: vlc_cond_timedwait" );
        vlc_cond_timedwait( &p_sys->wakeup_read_cond, &p_sys->buffer_mutex, mdate() + COND_POLL_TIME );
        // msg_Warn( p_stream, "ringbuf: WaitBufferForRead_l: vlc_cond_timedwait return" );
    }

    if( b_start_wait )
    {
        msg_RBuf( p_stream, "ringbuf: WaitBufferForRead_l(%d) wait end (%"PRId64" ms): %"PRId64" + %d",
                  i_read, (int64_t) ( mdate() - start_wait_time ) / 1000, p_sys->i_stream_offset,
                  p_sys->i_buffer_size );
    }
...
static int WaitBufferForWrite_l( stream_t *p_stream, int i_write )
{
    ...
    {
        if( StreamNeedAbort( p_stream ) ) {
            msg_Warn( p_stream, "ringbuf: WaitBufferForWrite_l need abort" );
            i_buffer = -1;
            break;
        }
        ...
        if( !b_start_wait )
        {
            msg_WBuf( p_stream, "ringbuf: WaitBufferForWrite_l(%d) wait start: %d",
        ...
        vlc_cond_broadcast( &p_sys->wakeup_read_cond );
        // msg_Warn( p_stream, "ringbuf: WaitBufferForWrite_l: vlc_cond_timedwait" );
        vlc_cond_timedwait( &p_sys->wakeup_write_cond, &p_sys->buffer_mutex, mdate() + COND_POLL_TIME );
        // msg_Warn( p_stream, "ringbuf: WaitBufferForRead_l: vlc_cond_timedwait end" );
    }

    if( b_start_wait )
    {
        msg_WBuf( p_stream, "ringbuf: WaitBufferForWrite_l(%d) wait end(%"PRId64" ms): %d",
                  i_buffer, (int64_t) ( mdate() - start_wait_time ) / 1000, p_sys->i_buffer_size );
    }
...
static void* BufferThread(void *p_this)
{
    ...
    mtime_t start_wait_time = mdate();
    mtime_t end_wait_time = mdate();
    mtime_t stream_read_used_time = 0;
    mtime_t stream_idle_used_time = 0;

    while( !StreamNeedAbort( p_stream ) )
    {
        ...
        if( p_sys->b_buffered_eos )
        {
            ...
            mutex_cleanup_push( &p_sys->buffer_mutex );
            p_sys->b_buffered_eos = true;
            while( !StreamNeedAbort( p_stream ) && !p_sys->b_seek_request )
            {
                // msg_Warn( p_stream, "ringbuf: BufferThread: vlc_cond_timedwait" );
                vlc_cond_timedwait( &p_sys->wakeup_write_cond, &p_sys->buffer_mutex, mdate() + COND_POLL_TIME );
                // msg_Warn( p_stream, "ringbuf: BufferThread: vlc_cond_timedwait end" );
            }
            vlc_cleanup_run( );

            if( StreamNeedAbort( p_stream ) ) {
                msg_Warn( p_stream, "ringbuf: BufferThread need abort" );
                break;
            }
        ...
        start_wait_time = mdate();
        stream_idle_used_time = start_wait_time - end_wait_time;
        if ( stream_idle_used_time >= MAX_DELAY_PER_READ)
            msg_Warn(p_stream, "BufferThread: idle too long (%d ms)", (int)(stream_idle_used_time / 1000));

        int i_read_ret = stream_Read( p_source, buf, i_step_read );

        end_wait_time = mdate();
        stream_read_used_time = end_wait_time - start_wait_time;
        if ( stream_read_used_time >= MAX_DELAY_PER_READ)
            msg_Warn(p_stream, "BufferThread: stream_Read too late (%d ms)", (int)(stream_read_used_time / 1000));
    ...
    msg_Warn( p_stream, "ringbuf: BufferThread return" );
    return NULL;
}
